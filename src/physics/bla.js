...
class Bla {
  ...
  constructor (trafficGraph, width, height, isEnabled) {
    ...
    this._hooksSprings = [];
    this._hooksSprings_restLength = 70;
  ...
  setOptions (options) {
    ...
    if (v !== SENTINEL) {
      if (typeof v !== 'number' || v < 0 || v > 1 || !canCoerceToFiniteDouble(v)) {
        Console.warn('Bla.setOptions: got invalid viscous drag coefficient. Expected number in range [0, 1], but got: ', v);
      } else {
    ...
    if (v !== SENTINEL) {
      if (objectGetPrototypeOf(v) !== objectPrototype) {
        Console.warn('Bla.setOptions(options): options.particles was expected to be a pojo, but got:', v);
      }
      v = getOwnProp(v, 'mass');
      if (v !== SENTINEL) {
        if (typeof v !== 'number' || v < 0.1 || !canCoerceToFiniteDouble(v)) {
          Console.warn('Bla.setOptions(options): got invalid options.particles.mass. Expected number in range [0.1, Inf), but got: ', v);
        } else {
    ...
    if (v !== SENTINEL) {
      ...
      if (objectGetPrototypeOf(optsHS) !== objectPrototype) {
        Console.warn('Bla.setOptions(options): options.hooksSpring was expected to be a pojo, but got:', optsHS);
      } else {
        let restLen = getOwnProp(optsHS, 'restLength');
        if (restLen !== SENTINEL) {
          if (typeof restLen !== 'number' || !canCoerceToFiniteDouble(restLen) || restLen < 0) {
            Console.warn('Bla.setOptions: hooksSpring.restLength was expected to be a non-negative finite number, but got:', restLen);
            restLen = SENTINEL;
          } else {
        ...
        if (springConst !== SENTINEL) {
          if (typeof springConst !== 'number' || !canCoerceToFiniteDouble(springConst) || springConst < 0 || springConst > 1) {
            Console.warn('Bla.setOptions: hooksSpring.restLength was expected to be a non-negative finite number, but got:', springConst);
        ...
        if (dampingConst !== SENTINEL) {
          if (typeof dampingConst !== 'number' || !canCoerceToFiniteDouble(dampingConst) || dampingConst < 0 || dampingConst > 1) {
            Console.warn('Bla.setOptions: hooksSpring.restLength was expected to be a non-negative finite number, but got:', dampingConst);
      ...
      Console.debug('Bla.setOptions: ', options, this.getOptions());
    }
  }
  ...
  update (time) {
    ...
    try {
      ...
      Bla.calculateDerivatives(simParticles);
  ...
  _calculateForces (simParticles, simHooksSprings) {
    ...
    for (const particle of simParticles) {
      particle.f.set(
