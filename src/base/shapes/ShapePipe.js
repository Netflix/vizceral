...
class ShapePipe {
  constructor () {
    ...
    this.customNode.material = this._createMaterial(GlobalStyles.styles.colorShapePipe);
  ...
  _createInnerGeometry (radius, curveSegments) {
    ...
    const polyShape = ShapesUtils.getShapeFromPolyPoints(rawPolyString, ' ', -21, -21);

    const holeShape = new THREE.Shape();
    holeShape.moveTo(radius / 2, 0);
    holeShape.absarc(6, -10, radius / 2, 0, 2 * Math.PI, false);
  ...
  _createOuterBorder (radius, curveSegments) {
    ...
    border.absarc(0, 0, radius / 3, 0, Math.PI * 2, false);
